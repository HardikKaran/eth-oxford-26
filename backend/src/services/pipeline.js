...
export async function executePipeline(params) {
  ...
  // Helper: attempt a blockchain call, log and skip if it fails
  async function tryChain(label, fn) {
    try {
      return await fn();
    } catch (err) {
      log.warn(`[${label}] On-chain call failed (continuing in simulation): ${err.message}`);
      return null;
    }
  }
  ...
  try {
    ...
    // Submit verification on-chain (non-blocking — pipeline continues even if this fails)
    await tryChain("verifyRequest", () =>
    ...
    if (!consensusResult.approved) {
      ...
      await tryChain("submitConsensus-reject", () =>
        blockchain.submitConsensusOnChain(
    ...
    await tryChain("submitConsensus-approve", () =>
      blockchain.submitConsensusOnChain(
        requestId,
        true,
    ...
    await tryChain("assignFulfiller", () =>
      blockchain.assignFulfillerOnChain(requestId, fulfillerAddr)
    );
